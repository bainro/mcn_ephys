...
OUT_DIR = Path(r"G:\\merged_16hr_SWS_run_CONNECTS_2")
ANIMAL_ID = "ms17"
...
# "balanced_random" reproduces the original null: choose 48 of 96 blocks uniformly.
# This mostly samples near 50/50 state mixtures.
#
# "stratified_state_separation" deliberately samples a broad range of state
# purity values by forcing the number of running blocks in A to span from
# nearly all-SWS to nearly all-running. This is better for asking whether
# state purity correlates with edge count / max-weight score.
RANDOM_SPLIT_MODE = "stratified_state_separation"

# Avoid exact state-pure random splits that duplicate the true split or its
# A/B reversal. With 48 running and 48 SWS blocks, this uses k=1..47 running
# blocks in A instead of k=0..48.
EXCLUDE_EXACT_TRUE_OR_REVERSED_RANDOM_SPLITS = True
...
def make_true_and_random_splits(
    total_h,
    boundary_h,
    block_min,
    n_random,
    seed,
    random_split_mode=RANDOM_SPLIT_MODE,
    exclude_exact_true_or_reversed=EXCLUDE_EXACT_TRUE_OR_REVERSED_RANDOM_SPLITS,
):
    ...
    if running_blocks.size != sws_blocks.size:
        print(
            ...
            f"{running_blocks.size} vs {sws_blocks.size}. Random splits will still be duration-matched."
        )
    ...
    if running_blocks.size < half_n or sws_blocks.size < half_n:
        raise ValueError(
            "This broad-purity splitter assumes the first state and second state each have at least "
            f"{half_n} blocks. Got running={running_blocks.size}, SWS={sws_blocks.size}, half_n={half_n}."
        )

    true_A = np.sort(running_blocks[:half_n]).astype(int)
    true_B = np.sort(sws_blocks[:half_n]).astype(int)

    split_specs = [
        {
            "split_id": "true_running_vs_sws",
            "split_type": "true_state",
            "blocks_A": true_A.tolist(),
            "blocks_B": true_B.tolist(),
            "label_A": RUNNING_LABEL,
            "label_B": SWS_LABEL,
            "target_running_blocks_A": int(half_n),
        }
    ]

    rng = np.random.default_rng(int(seed))
    mode = str(random_split_mode).strip().lower()

    # Track exact A block sets to avoid duplicate random splits.
    true_key = tuple(sorted(true_A.tolist()))
    reversed_key = tuple(sorted(true_B.tolist()))
    seen = {true_key, reversed_key}

    if mode == "balanced_random":
        # Original behavior: choose duration-matched halves uniformly from all blocks.
            ...
            split_specs.append(
                {
                    ...
                    "label_A": "random_A",
                    "label_B": "random_B",
                    "target_running_blocks_A": int(sum(np.isin(a, running_blocks))),
                }
            )

    elif mode in {"stratified_state_separation", "broad_purity", "stratified_purity"}:
        # Broad-purity behavior:
        # Let k = number of running blocks in A. A always has half_n blocks total:
        #   A = k running blocks + (half_n-k) SWS blocks.
        # B is the complement. This makes state_separation span almost 0..1:
        #   state_separation = abs(k/half_n - (half_n-k)/half_n).
        if exclude_exact_true_or_reversed:
            k_values = np.arange(1, half_n, dtype=int)  # avoids k=0 and k=half_n exact pure/inverted
        else:
            k_values = np.arange(0, half_n + 1, dtype=int)

        if k_values.size == 0:
            raise ValueError("No valid k_values for stratified state separation.")

        # Make target k values approximately uniform across the full purity range.
        # For 60 splits and 47 k-values, this samples the whole axis once, then fills extras.
        target_ks = []
        while len(target_ks) < int(n_random):
            shuffled = k_values.copy()
            rng.shuffle(shuffled)
            target_ks.extend(shuffled.tolist())
        target_ks = target_ks[: int(n_random)]
        ...
        for i, k in enumerate(target_ks):
            ...
            for _attempt in range(10000):
                run_part = rng.choice(running_blocks, size=n_run_A, replace=False) if n_run_A else np.array([], dtype=int)
                sws_part = rng.choice(sws_blocks, size=n_sws_A, replace=False) if n_sws_A else np.array([], dtype=int)
                a = np.sort(np.concatenate([run_part, sws_part]).astype(int))
                key = tuple(a.tolist())
                if key not in seen:
                    seen.add(key)
                    break
            else:
                raise RuntimeError(f"Could not generate a unique broad-purity split for k={k} after many attempts.")

            b = np.setdiff1d(blocks, a, assume_unique=False).astype(int)
            split_specs.append(
                {
                    "split_id": f"random_{i:03d}_kRunA{k:02d}",
                    "split_type": "random",
                    "blocks_A": a.tolist(),
                    "blocks_B": b.tolist(),
                    "label_A": "random_A",
                    "label_B": "random_B",
                    "target_running_blocks_A": int(k),
                }
            )

    else:
        raise ValueError(
            "random_split_mode must be 'balanced_random' or 'stratified_state_separation'. "
            f"Got {random_split_mode!r}."
        )
...
def score_split(split_spec, result_A, result_B, cluster_ids, block_table):
    ...
    summary = {
        ...
        "target_running_blocks_A": split_spec.get("target_running_blocks_A", np.nan),
        **state_counts,
        "n_cells": int(n_cells),
...
def make_plots(summary_df: pd.DataFrame, out_dir: Path):
    ...
    for col, ylabel, stem in [
        ...
        ("n_union_edges", "Detected connections (union edges)", "state_separation_vs_detected_connections"),
    ]:
        fig, ax = plt.subplots(figsize=(8.5, 5.5))
        ax.scatter(rand["state_separation"], rand[col], s=40, alpha=0.75, label="random splits")
        ax.scatter(true_row["state_separation"], true_row[col], s=120, marker="*", label="true running vs SWS")
        ax.set_xlabel("State separation")
        ax.set_ylabel(ylabel)
        ax.set_title("State purity vs FSC output")
        ax.legend()
    ...
    # Same detected-connections relationship on the pseudocount SWS/running ratio axis.
    fig, ax = plt.subplots(figsize=(8.5, 5.5))
    ax.scatter(
        rand["max_sws_to_running_ratio_pseudocount"],
        rand["n_union_edges"],
        s=40,
        alpha=0.75,
        label="random splits",
    ...
    ax.scatter(
        true_row["max_sws_to_running_ratio_pseudocount"],
        true_row["n_union_edges"],
        s=120,
        marker="*",
        label="true running vs SWS",
    )
    ax.set_xscale("log")
    ax.set_xlabel("Split pureness: max(SWS / running ratio, pseudocount)")
    ax.set_ylabel("Detected connections (union edges)")
    ax.set_title("State pureness vs detected FSC connections")
    ax.legend()
    ax.grid(True, alpha=0.25)
    fig.tight_layout()
    png = out_dir / "pureness_ratio_vs_detected_connections.png"
    pdf = out_dir / "pureness_ratio_vs_detected_connections.pdf"
    fig.savefig(png, dpi=250, bbox_inches="tight")
    fig.savefig(pdf, bbox_inches="tight")
    plt.close(fig)
    saved.extend([png, pdf])

    # Histogram of detected connections.
    fig, ax = plt.subplots(figsize=(8.5, 5.5))
    ax.hist(rand["n_union_edges"].to_numpy(dtype=float), bins=25, alpha=0.8)
    ax.axvline(float(true_row["n_union_edges"]), linewidth=2.5, linestyle="--", label="true running vs SWS")
    ax.set_xlabel("Detected connections (union edges)")
    ax.set_ylabel("Random split count")
    ax.set_title("True state split vs random splits\nDetected connections")
    ax.legend()
    fig.tight_layout()
    png = out_dir / "detected_connections_hist.png"
    pdf = out_dir / "detected_connections_hist.pdf"
    fig.savefig(png, dpi=250, bbox_inches="tight")
    fig.savefig(pdf, bbox_inches="tight")
    plt.close(fig)
    saved.extend([png, pdf])
...
def run_analysis(
    ...
    seed: int,
    save_matrices: bool,
    random_split_mode: str = RANDOM_SPLIT_MODE,
    exclude_exact_true_or_reversed_random_splits: bool = EXCLUDE_EXACT_TRUE_OR_REVERSED_RANDOM_SPLITS,
):
...
def parse_args():
    ...
    p.add_argument(
        "--random-split-mode",
        type=str,
        default=RANDOM_SPLIT_MODE,
        choices=["balanced_random", "stratified_state_separation"],
        help="balanced_random = original near-50/50 null; stratified_state_separation = broad purity range.",
    )
    p.add_argument(
        "--include-exact-pure-random-splits",
        action="store_true",
        help="Include k=0 and k=half_n random splits, which duplicate the true split or its A/B reversal when states are perfectly 50/50.",
    )
...
def main():
    ...
    run_analysis(
        ...
        random_split_mode=args.random_split_mode,
        exclude_exact_true_or_reversed_random_splits=(not args.include_exact_pure_random_splits),
    )
