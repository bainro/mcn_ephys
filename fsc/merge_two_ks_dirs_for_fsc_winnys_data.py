...
# actually used a different version for winny's ultimately. It was named merge_two_ks_dirs_for_fsc.py at time of writing this.
...
10. Probe 2 x coordinates are shifted so that:
      median(probe2 unit x) - median(probe1 unit x) == user-provided separation in um
   For example, enter 2000 for probe 2 center being 2 mm to the right of probe 1.
11. This script also rebuilds channel_positions.npy from CHMAPFULL_PATH when it can
...
12. This script only writes to MERGED_KS_DIR. It does not modify KS_DIR_1,
...
13. The merged spike_times.npy dtype is preserved from the original KS files.
...
# -----------------------------
KS_DIR_1 = Path(f"Y:\Winny\FinishedPhy\TR15_S1_spikesorted_512ch_10-10-2023_withShift_DONE")
KS_DIR_2 = Path(f"Y:\Winny\FinishedPhy\TR15_HPC_spikesorted_512ch_10-10-2023_withShift_DONE")

MAT_PATH_1 = Path(f"Y:\Winny\FinishedPhy\TR15_S1_spikesorted_512ch_10-10-2023_withShift_DONE\Analysis\TR15_S1_unitdata_CQ.mat")
MAT_PATH_2 = Path(f"Y:\Winny\FinishedPhy\TR15_HPC_spikesorted_512ch_10-10-2023_withShift_DONE\Analysis\TR15_PPC_HPC_unitdata_CQ.mat")

MERGED_KS_DIR = Path(f"E:\\rob\\merged_TR15_S1_V1")
...
CASE_SENSITIVE_REGION = True

# If None, script asks interactively.
PROBE2_CENTER_X_MINUS_PROBE1_CENTER_X_UM = None
...
def main():
    ...
    center1 = float(np.median(units1["x_um_unshifted"].to_numpy(dtype=float)))
    center2 = float(np.median(units2["x_um_unshifted"].to_numpy(dtype=float)))
