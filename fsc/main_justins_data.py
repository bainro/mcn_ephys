...
def _adaptive_bin_distance_probability(
    df: pd.DataFrame,
    base_bin_um: float = 100.0,
    min_possible_per_bin: int = 10_000,
):
    """
    Distance-bin probability using adjacent base bins merged until each bin has
    at least `min_possible_per_bin` possible directed pairs when possible.

    This is mainly for the different-probe far tail, where fixed 100 um bins can
    have tiny denominators and visually noisy rates. A small final remainder is
    merged into the previous bin instead of being plotted as a fragile point.
    """
    if df.empty:
        return pd.DataFrame()
    ...
    vals = work["distance_um"].astype(float).to_numpy()
    start = np.floor(float(np.nanmin(vals)) / base_bin_um) * base_bin_um
    stop = np.ceil(float(np.nanmax(vals)) / base_bin_um) * base_bin_um + base_bin_um
    edges = np.arange(start, stop + 0.5 * base_bin_um, base_bin_um, dtype=float)
    if edges.size < 2:
        edges = np.asarray([start, start + base_bin_um], dtype=float)

    fixed = _bin_distance_probability(work, edges, min_possible_per_bin=1)
    if fixed.empty:
        return fixed

    # Merge adjacent fixed-width bins until each displayed bin has enough
    # opportunities. This preserves distance order while avoiding sparse tail
    # bins such as n=2 or n=134 from dominating the visual narrative.
    rows = []
    acc = None

    def start_acc(row):
        return {
            "distance_bin_low_um": float(row["distance_bin_low_um"]),
            "distance_bin_high_um": float(row["distance_bin_high_um"]),
            "n_possible_directed": int(row["n_possible_directed"]),
    ...
    for _, row in fixed.iterrows():
        if acc is None:
            ...
            acc["distance_bin_high_um"] = float(row["distance_bin_high_um"])
            acc["n_possible_directed"] += int(row["n_possible_directed"])
            acc["n_detected"] += int(row["n_detected"])

        if acc["n_possible_directed"] >= int(min_possible_per_bin):
            rows.append(acc)
            acc = None

    if acc is not None:
        if rows:
            rows[-1]["distance_bin_high_um"] = acc["distance_bin_high_um"]
            rows[-1]["n_possible_directed"] += acc["n_possible_directed"]
            rows[-1]["n_detected"] += acc["n_detected"]
        else:
            rows.append(acc)

    out_rows = []
    for r in rows:
        possible = int(r["n_possible_directed"])
        detected = int(r["n_detected"])
        lo, hi = _wilson_ci(np.asarray([detected]), np.asarray([possible]))
        low = float(r["distance_bin_low_um"])
        high = float(r["distance_bin_high_um"])
        in_bin = work[(work["distance_um"].astype(float) >= low) & (work["distance_um"].astype(float) < high)]
        out_rows.append(
            {
                "distance_bin_low_um": low,
                "distance_bin_high_um": high,
                "distance_bin_center_um": float((low + high) / 2.0),
                "distance_bin_width_um": float(high - low),
                "n_possible_directed": possible,
                "n_detected": detected,
                "connection_probability": float(detected / possible) if possible else np.nan,
                "connection_probability_percent": float(100.0 * detected / possible) if possible else np.nan,
                "wilson_ci_low_percent": float(100.0 * lo[0]),
                "wilson_ci_high_percent": float(100.0 * hi[0]),
                "n_animals_contributing": int(in_bin["animal_id"].nunique()) if "animal_id" in in_bin.columns else np.nan,
                "adaptive_base_bin_um": float(base_bin_um),
                "adaptive_min_possible_per_bin": int(min_possible_per_bin),
            }
        )
...
def _adaptive_merge_binned_probability(
    fixed_binned: pd.DataFrame,
    min_possible_per_bin: int = 25_000,
):
    """
    Merge adjacent already-binned distance rows until each displayed bin has
    at least `min_possible_per_bin` possible directed pairs when possible.

    This preserves custom/high-resolution near-field bins such as 0-25, 25-50,
    50-75 while widening bins where directed-pair opportunities become sparse.
    A small final remainder is merged backward into the previous bin.
    """
    if fixed_binned is None or fixed_binned.empty:
        return pd.DataFrame()

    work = fixed_binned.copy().sort_values("distance_bin_low_um").reset_index(drop=True)
    rows = []
    acc = None
    ...
    for _, row in work.iterrows():
        ...
        if "n_animals_contributing" in row and pd.notna(row["n_animals_contributing"]):
            # The source CSV stores only counts, not identities. Keep max count as a
            # conservative/display value for the merged row.
            acc.setdefault("n_animals_contributing_values", []).append(int(row["n_animals_contributing"]))
    ...
    if acc is not None:
        if rows:
            ...
            rows[-1]["n_possible_directed"] += acc["n_possible_directed"]
            rows[-1]["n_detected"] += acc["n_detected"]
            rows[-1]["source_bins"] += acc["source_bins"]
            rows[-1].setdefault("n_animals_contributing_values", []).extend(acc.get("n_animals_contributing_values", []))
        else:
            rows.append(acc)
    ...
    for r in rows:
        ...
        detected = int(r["n_detected"])
        lo, hi = _wilson_ci(np.asarray([detected]), np.asarray([possible]))
        p = float(detected / possible) if possible else np.nan
        animal_counts = r.get("n_animals_contributing_values", [])
        out_rows.append(
            {
                "distance_bin_low_um": float(r["distance_bin_low_um"]),
                "distance_bin_high_um": float(r["distance_bin_high_um"]),
                "distance_bin_center_um": float((float(r["distance_bin_low_um"]) + float(r["distance_bin_high_um"])) / 2.0),
                "distance_bin_width_um": float(float(r["distance_bin_high_um"]) - float(r["distance_bin_low_um"])),
                "n_possible_directed": possible,
                "n_detected": detected,
                "connection_probability": p,
                "connection_probability_percent": float(100.0 * p) if possible else np.nan,
                "wilson_ci_low_percent": float(100.0 * lo[0]),
                "wilson_ci_high_percent": float(100.0 * hi[0]),
                "binomial_se_percent": float(100.0 * np.sqrt(p * (1.0 - p) / possible)) if possible else np.nan,
                "n_animals_contributing": int(max(animal_counts)) if animal_counts else np.nan,
                "adaptive_min_possible_per_bin": int(min_possible_per_bin),
                "source_bins": int(r.get("source_bins", 1)),
            }
        )
    return pd.DataFrame(out_rows)


def _save_full_scope_log_distance_plot(
    binned: pd.DataFrame,
    out_path: Path,
    include_wilson95: bool,
    title: str | None = None,
):
    """
    Save a publication/grant-friendly full-distance plot on a log y-axis.

    The line shows pooled opportunity-weighted connection probability from
    adaptively merged distance bins. Datapoints are always shown. If
    include_wilson95 is True, error bars show Wilson 95% CIs; otherwise only
    the line and datapoints are plotted.
    """
    ...
    x = binned.loc[positive, "distance_bin_center_um"].to_numpy(dtype=float)
    y = y[positive]
    plot_df = binned.loc[positive].copy()
    ...
    try:
        ...
        if include_wilson95:
            yerr = _nonnegative_yerr_from_ci(
        y,
        plot_df["wilson_ci_low_percent"].to_numpy(dtype=float),
        plot_df["wilson_ci_high_percent"].to_numpy(dtype=float),
    )
            ax.errorbar(
                x,
                y,
        ...
        ax.set_xlabel("Distance bin center (um)", labelpad=14)
        ax.set_ylabel("Detected directed pairs (% of possible)", labelpad=16)
        if title:
    ...
    return {"png": out_path, "pdf": pdf_path}


def _nonnegative_yerr_from_ci(y, ci_low, ci_high):
    """Matplotlib-safe asymmetric yerr from lower/upper CI bounds."""
    ...
    # Guard against tiny floating-point negatives, NaNs, or malformed bins.
...
def save_aggregate_distance_probability_plots(plans, aggregate_out_dir: Path, prefix: str = "aggregate_distance"):
    ...
    # Grant/presentation version: preserve the custom high-resolution near-field
    # bins, but adaptively merge adjacent bins until each displayed bin has at
    # least 25k directed-pair opportunities. This stabilizes the long-range tail
    # without needing a separate zoom panel.
    all_adaptive_binned = _adaptive_merge_binned_probability(
        all_binned,
        min_possible_per_bin=25_000,
    )
    all_adaptive_binned_path = aggregate_out_dir / f"{prefix}_binned_probability_all_pairs_adaptive25k.csv"
    all_adaptive_binned.to_csv(all_adaptive_binned_path, index=False)
    ...
    tail_fixed_binned_path = None
    if not tail_pairs.empty:
        # Save the original 100 um fixed bins for audit/debugging, but plot and
        # emphasize the adaptive/min-count version to avoid fragile sparse tail points.
        tail_edges = _distance_bin_edges_for_values(tail_pairs["distance_um"].to_numpy(dtype=float), tail_only=True)
        tail_fixed_binned = _bin_distance_probability(tail_pairs, tail_edges, min_possible_per_bin=1)
        tail_fixed_binned_path = aggregate_out_dir / f"{prefix}_binned_probability_different_probe_tail_fixed100um.csv"
        tail_fixed_binned.to_csv(tail_fixed_binned_path, index=False)

        tail_binned = _adaptive_bin_distance_probability(
            tail_pairs,
            base_bin_um=100.0,
            min_possible_per_bin=10_000,
        )
        tail_binned_path = aggregate_out_dir / f"{prefix}_binned_probability_different_probe_tail_adaptive.csv"
    ...
    if not all_binned.empty:
        ...
        yerr = _nonnegative_yerr_from_ci(
        y,
        all_binned["wilson_ci_low_percent"].to_numpy(dtype=float),
        all_binned["wilson_ci_high_percent"].to_numpy(dtype=float),
    )
        ax.errorbar(
    ...
    if not tail_binned.empty:
        ...
        yerr = _nonnegative_yerr_from_ci(
        y,
        tail_binned["wilson_ci_low_percent"].to_numpy(dtype=float),
        tail_binned["wilson_ci_high_percent"].to_numpy(dtype=float),
    )
        x = tail_binned["distance_bin_center_um"].to_numpy(dtype=float)
        xerr = None
        if "distance_bin_width_um" in tail_binned.columns:
            xerr = tail_binned["distance_bin_width_um"].to_numpy(dtype=float) / 2.0
        ax.errorbar(
            x,
            y,
            xerr=xerr,
    ...
    ax.set_xlabel("Distance between unit peak channels (um)")
    ax.set_ylabel("Detected directed pairs (% of possible)")
    ax.set_title("Far tail: different-probe pairs only (adaptive bins)")
    ax.grid(True, alpha=0.25)
    ...
    if not all_binned.empty:
        ...
        for ax, binned, title in [
            ...
            (axes[1], tail_binned, "Far tail: different-probe pairs only (adaptive bins)"),
        ]:
    ...
    # Single-panel full-range log-y figures for grant panels. Both have datapoints.
    # One includes Wilson 95% CIs; the other shows the clean trend without error bars.
    full_log_wilson_paths = _save_full_scope_log_distance_plot(
        all_adaptive_binned,
        aggregate_out_dir / f"{prefix}_probability_full_scope_logy_wilson95_points.png",
        include_wilson95=True,
        title=None,
    )
    full_log_noerr_paths = _save_full_scope_log_distance_plot(
        all_adaptive_binned,
        aggregate_out_dir / f"{prefix}_probability_full_scope_logy_points_no_error.png",
        include_wilson95=False,
        title=None,
    )

    print(f"  saved aggregate directed pair rows: {all_rows_path}")
    print(f"  saved aggregate distance bins: {all_binned_path}")
    print(f"  saved aggregate adaptive 25k all-pairs bins: {all_adaptive_binned_path}")
    if tail_fixed_binned_path is not None:
        print(f"  saved aggregate different-probe fixed 100 um tail bins: {tail_fixed_binned_path}")
    if tail_binned_path is not None:
        print(f"  saved aggregate different-probe adaptive tail bins: {tail_binned_path}")
    ...
    if full_log_wilson_paths is not None:
        print(f"  saved full-scope log-y Wilson 95% plot: {full_log_wilson_paths['png']}")
        print(f"  saved full-scope log-y Wilson 95% PDF: {full_log_wilson_paths['pdf']}")
    if full_log_noerr_paths is not None:
        print(f"  saved full-scope log-y no-error plot: {full_log_noerr_paths['png']}")
        print(f"  saved full-scope log-y no-error PDF: {full_log_noerr_paths['pdf']}")

    return {
        "all_pair_rows_csv": all_rows_path,
        "all_binned_csv": all_binned_path,
        "all_adaptive_25k_binned_csv": all_adaptive_binned_path,
        "tail_binned_csv": tail_binned_path,
        "tail_fixed_100um_binned_csv": tail_fixed_binned_path,
        "distance_plot_png": plot_path,
        "distance_log_plot_png": log_plot_path,
        "full_scope_log_wilson95_png": full_log_wilson_paths["png"] if full_log_wilson_paths else None,
        "full_scope_log_wilson95_pdf": full_log_wilson_paths["pdf"] if full_log_wilson_paths else None,
        "full_scope_log_no_error_png": full_log_noerr_paths["png"] if full_log_noerr_paths else None,
        "full_scope_log_no_error_pdf": full_log_noerr_paths["pdf"] if full_log_noerr_paths else None,
    }
