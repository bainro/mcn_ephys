...
def assign_shanks_from_ks_channel_map(
    ...
    plot_x = channel_positions[:, 0] if channel_positions is not None else meta["x_um"].to_numpy()
    plot_y = channel_positions[:, 1] if channel_positions is not None else meta["y_um"].to_numpy()
    x_span = float(np.ptp(plot_x)) if len(plot_x) else 1.0
    y_span = float(np.ptp(plot_y)) if len(plot_y) else 1.0
    fig_w = max(5.0, min(20.0, 5.0 * max(1.0, x_span / max(y_span, 1.0))))
    ...
    ax.set_title("Probe geometry: choose x split(s) for shank/region labels")
    ax.axis("equal")
...
def plot_cross_shank_geometry_connections(
    ...
    x_span = float(np.ptp(plot_x)) if len(plot_x) else 1.0
    y_span = float(np.ptp(plot_y)) if len(plot_y) else 1.0
    fig_w = max(6.0, min(20.0, 7.0 * max(1.0, x_span / max(y_span, 1.0))))
